...
// providers, groups, …) added for the fleet client-flip: routing, auth/scope
...
describe("cloud service generic resources", () => {
  test("migration 0005 is registered", () => {
    expect(maileryCloudMigrations().some((m) => m.id === "0005_mailery_selfhosted_resources")).toBe(true);
  });

  test("GET /v1/contacts requires auth (401)", async () => {
    const res = await handleCloudRequest(deps(), req("GET", "/v1/contacts"));
    expect(res?.status).toBe(401);
  });

  test("read scope can GET but not POST contacts", async () => {
    const d = deps();
    const list = await handleCloudRequest(d, req("GET", "/v1/contacts", { token: readToken() }));
    expect(list?.status).toBe(200);
    expect((await list!.json()).items).toEqual([]);
    const post = await handleCloudRequest(d, req("POST", "/v1/contacts", { token: readToken(), body: { email: "a@x.com" } }));
    expect(post?.status).toBe(403);
  });

  test("create -> list -> get -> delete round-trips a contact", async () => {
    const d = deps();
    const create = await handleCloudRequest(d, req("POST", "/v1/contacts", { token: writeToken(), body: { email: "a@x.com", name: "A", send_count: 2, suppressed: true } }));
    ...
    const list = await handleCloudRequest(d, req("GET", "/v1/contacts", { token: writeToken() }));
    expect((await list!.json()).items).toHaveLength(1);

    const get = await handleCloudRequest(d, req("GET", `/v1/contacts/${created.id}`, { token: writeToken() }));
    expect(get?.status).toBe(200);
    expect((await get!.json()).email).toBe("a@x.com");

    const del = await handleCloudRequest(d, req("DELETE", `/v1/contacts/${created.id}`, { token: writeToken() }));
    expect(del?.status).toBe(200);
    const get2 = await handleCloudRequest(d, req("GET", `/v1/contacts/${created.id}`, { token: writeToken() }));
    expect(get2?.status).toBe(404);
  });

  test("send-keys (hyphenated path) and scheduled resolve and never expose secrets", async () => {
    const d = deps();
    const create = await handleCloudRequest(d, req("POST", "/v1/send-keys", { token: writeToken(), body: { owner_id: "o1", prefix: "esk_abc", label: "ci" } }));
    expect(create?.status).toBe(201);
    const row = await create!.json();
    expect(row).not.toHaveProperty("key_hash");
    const sched = await handleCloudRequest(d, req("POST", "/v1/scheduled", { token: writeToken(), body: { subject: "hi", to_addresses: ["b@x.com"], status: "pending" } }));
    expect(sched?.status).toBe(201);
    expect((await sched!.json()).to_addresses).toEqual(["b@x.com"]);
  });

  test("unknown resource still 404s", async () => {
    const res = await handleCloudRequest(deps(), req("GET", "/v1/nonsense", { token: readToken() }));
