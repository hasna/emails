...
/**
 * 0021 — fleet-principal → tenant map (ADR-0001 Phase 1, additive only).
 *
 ...
 * SELF_HOSTED_RESOURCES and from the RLS list in 0013. Maps a verified fleet
 * token's `sub` (IdP user or service-principal id) to the emails tenant it may
 * act in. `fleet_tid` pins the IdP tenant observed when the grant was made — a
 ...
 */
const FLEET_PRINCIPAL_TENANTS = defineMigration(
  "0021_fleet_principal_tenants",
  `
  CREATE TABLE IF NOT EXISTS fleet_principal_tenants (
  ...
  CREATE INDEX IF NOT EXISTS fleet_principal_tenants_tenant_idx ON fleet_principal_tenants (tenant_id);
  `,
);
