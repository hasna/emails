...
import { CloudHttpError, resetCloudConfigCache } from "./cloud-store.js";
...
const server = Bun.serve({ port: 0, fetch(req) {
  ...
  if (p === "/v1/contacts") return ok({ items: [{ id: "c1", email: "cloud@example.com", name: "Cloud", send_count: 3, suppressed: false, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-02T00:00:00Z" }] });
  if (p === "/v1/groups") return ok({ items: [{ id: "g1", name: "cloud-group", description: null, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" }] });
  if (p === "/v1/owners") return ok({ items: [{ id: "o1", type: "agent", name: "Cloud Agent", contact_email: null, external_id: null, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" }] });
  if (p === "/v1/providers") return ok({ items: [{ id: "p1", name: "cloud-ses", type: "ses", region: "us-east-1", active: true, created_at: "2026-01-01T00:00:00Z", updated_at: "2026-01-01T00:00:00Z" }] });
...
describe("resource repos route reads to cloud in cloud mode", () => {
  test("listContacts returns cloud rows", () => {
    ...
    expect(rows[0]!.email).toBe("cloud@example.com");
    expect(rows[0]!.send_count).toBe(3);
  });

  test("listGroups returns cloud rows", () => {
    expect(listGroups().map((g) => g.name)).toEqual(["cloud-group"]);
  });

  test("listOwners returns cloud rows and filters by type", () => {
    expect(listOwners().map((o) => o.id)).toEqual(["o1"]);
    expect(listOwners("agent").map((o) => o.id)).toEqual(["o1"]);
    expect(listOwners("human")).toEqual([]);
  });

  test("listProviderSummaries returns cloud rows without secrets", () => {
    const rows = listProviderSummaries();
    expect(rows).toHaveLength(1);
    expect(rows[0]!.name).toBe("cloud-ses");
    expect(rows[0]).not.toHaveProperty("api_key");
  });

  test("listScheduledEmails returns cloud rows", () => {
  ...
  test("missing endpoint FAILS CLOSED (no silent local read)", () => {
    expect(() => listTemplates()).toThrow(CloudHttpError);
  });
});
