...
describe("verification code extraction", () => {
  // PURE: extractVerificationCodes does no I/O.
  ...
  // PURE: findVerificationCode is a matcher over whatever candidates it is given.
  ...
  // STUB: the synchronous SQLite candidate reader has no local store in the
  // self-hosted client. It fails loud rather than returning an empty list;
  // callers must use resolveMailDataSource().verificationCandidates(...) over /v1.
  // The old test validated the removed local SQL shape (inbound_recipients join,
  // is_sent/is_archived predicates) — that behavior no longer exists.
  it("throws because the local candidate reader is unavailable in the self-hosted client", () => {
    expect(() => listVerificationCodeCandidates("me@example.com", { limit: 1 })).toThrow(
