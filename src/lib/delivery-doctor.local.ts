...
import { normalizeEmailAddress } from "../db/inbound.local.js";
...
import { listAddressOwnershipEvents } from "../db/owners.local.js";
...
export function diagnoseInboundDelivery(address: string, db: Database = getDatabase()): DeliveryDoctorReport {
  ...
  const enrichedExactAddresses = exactAddresses.length > 0 ? enrichAddresses(exactAddresses) : [];
