...
export type EmailsAiProvider = "cerebras" | "groq";
...
export function normalizeEmailsAiProvider(value: string | undefined): EmailsAiProvider {
...
export function resolveEmailsAiDefaults(opts?: {
  provider?: EmailsAiProvider;
  model?: string | null;
  defaultProvider?: EmailsAiProvider;
  defaultGroqModel?: string;
  defaultCerebrasModel?: string;
}): { provider: EmailsAiProvider; model: string } {
  const config = loadConfig();
  const fallbackProvider = opts?.defaultProvider ?? "cerebras";
  const provider = normalizeEmailsAiProvider(
    opts?.provider
...
export function getEmailsAiApiKey(provider: EmailsAiProvider): string {
  const config = loadConfig();
...
export async function createEmailsAiModel(provider: EmailsAiProvider, model: string): Promise<unknown> {
  const apiKey = getEmailsAiApiKey(provider);
