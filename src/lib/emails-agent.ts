...
import {
  ...
  resolveEmailsAiDefaults,
  type EmailsAiProvider,
} from "./emails-ai.js";

export type EmailsAgentProvider = EmailsAiProvider;

export interface EmailsAgentOptions {
  provider?: EmailsAgentProvider;
  model?: string;
  maxSteps?: number;
...
export interface EmailsAgentResult {
  text: string;
  provider: EmailsAgentProvider;
  model: string;
  tools: string[];
...
export const EMAILS_AGENT_SYSTEM_PROMPT = `You are Emails's read-only email agent.

You help inspect local Emails email data. You may list, search, read, summarize, and extract links from local emails using the tools provided.
...
export function resolveEmailsAgentDefaults(opts?: Pick<EmailsAgentOptions, "provider" | "model">): { provider: EmailsAgentProvider; model: string } {
...
export function buildReadOnlyEmailsTools(db: Database = getDatabase()) {
  return {
    list_recent_emails: tool({
      description: "List recent inbound emails from the local Emails SQLite database. Returns summaries, not full bodies.",
    ...
    read_email: tool({
      description: "Read one inbound email by full or partial Emails id. Returns metadata and a truncated body.",
      inputSchema: z.object({
    ...
    extract_links: tool({
      description: "Extract links from one inbound email by full or partial Emails id.",
      inputSchema: z.object({
        id: z.string().min(1).describe("Full or partial inbound email id"),
    ...
    emails_status: tool({
      description: "Return a read-only health/status snapshot for local Emails providers, inboxes, domains, and addresses.",
      inputSchema: z.object({}),
...
export async function runEmailsAgent(prompt: string, opts: EmailsAgentOptions = {}, deps: GenerateTextDeps = {}): Promise<EmailsAgentResult> {
  const normalizedPrompt = prompt.trim();
  if (!normalizedPrompt) throw new Error("Agent prompt is required.");
  const { provider, model } = resolveEmailsAgentDefaults(opts);
  const tools = buildReadOnlyEmailsTools(opts.db);
  ...
  const result = await generateText({
    model: languageModel,
    system: `${EMAILS_AGENT_SYSTEM_PROMPT}\n\nAvailable tools: ${Object.keys(tools).join(", ")}.`,
...
export function formatEmailsAgentResult(result: EmailsAgentResult): string {
