...
describe("domain dns command", () => {
  ...
  // A provider that legitimately requires nothing (sandbox returns an empty
  // list) produced a bare "No DNS records found." — indistinguishable from a
  // lookup that failed.
  it("says which provider requires no records instead of printing an empty table", async () => {
    await stub.seed({
      providers: [{ id: "prov-sandbox", name: "sandbox", type: "sandbox", region: null, active: true }],
    });
    const result = await runDomainCommand(["domain", "dns", "example.com", "--provider", "prov-sandbox"]);
    expect(result.data).toMatchObject({ provider_id: "prov-sandbox", records: [] });
    expect(result.out).toContain("requires no DNS records for this domain");
    expect(result.out).toContain("Drop --provider to see the generic SPF/DMARC pair instead.");
  });
