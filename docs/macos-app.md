# Emails - macOS desktop app

Emails ships a native macOS desktop app alongside the `@hasna/emails` CLI. It is a UI
...
from the local Emails SQLite store.

It lives in the SAME repo as the CLI. The Swift module and target names that still start with `Mailery` are internal legacy names retained for this patch; the user-facing app and package name is Emails.

```
Package.swift                 swift-tools 6.0, platforms [.macOS("26.0")]
Sources/MaileryCore/ (legacy target name)          read the SQLite store + build the mutation CLI argv
Sources/MaileryApp/ (legacy target name)           the WKWebView host (AppKit) — injects __BOOT__, bridges `mail`
Sources/MailerySmoke/ (legacy target name)         CLI smoke harness (TDD; XCTest is unavailable under CLT)
web/                          the 3-pane mail UI (index.html / styles.css / app.js)
scripts/build_emails_app.sh  build + assemble dist/Emails.app (run on a Mac)
...
Emails deliberately splits reads from writes:

- **Reads go straight to SQLite (read-only).** the internal legacy `MaileryCore.MailStore` opens
...
- **Writes ALWAYS go through the `emails` CLI.** Every mutation — send, reply, mark-read,
  ...
  the shared DB. the internal legacy `MaileryCore.MaileryCLI` builds the exact argv (pure + unit-tested) and
  shells out.
...
App-level UI state belongs under `~/.hasna/apps/emails/` (e.g. theme is currently in
`localStorage`). The mail data itself is **not** owned by the app — it lives in the shared
`~/.hasna/emails/emails.db` and is managed by the `emails` CLI.
...
bash scripts/build_emails_app.sh      # assembles dist/Emails.app (bundle id com.hasna.emails)
...
log show --last 2m --predicate 'eventMessage CONTAINS "Emails:"' --info
#   → "Emails: rendered N thread rows"
```
...
```bash
ditto -c -k --keepParent dist/Emails.app dist/Emails.app.zip
attachments upload dist/Emails.app.zip
```
