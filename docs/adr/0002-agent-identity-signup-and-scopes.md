...
- Depends on: [ADR-0001 — Adopt `@hasna/tenants` as the identity authority](0001-adopt-fleet-idp-for-identity.md)
...
identity**, issued by the fleet IdP, never a copy of anyone's credential
bundle.
...
```
 ...
 operator ──► tenants CLI/API ──► service_principal ──► fleet token (pt=service,
                 │                 in @hasna/tenants      aud=emails, scope=[…],
                 │                                        sub=<principal-id>, ≤24h)
                 │ (3) map — one deliberate grant                │
                 └──► emails: fleet_principal_tenants row        │
...
1. **Create the agent's identity in the IdP** (owning tenant, display name,
   ...
   exchanges it daily for short-lived fleet tokens; it never holds a password
...
2. **Mint a fleet token** for `--app emails`. Claims per the ADR-0001 wire
...
3. **Grant mail access in emails** — an explicit mapping row, created by an
   ...
   ```
   emails auth fleet map <sub> [--tenant <slug>] [--note <text>]
   emails auth fleet list
   emails auth fleet revoke <sub>
   ```
...
4. **The agent uses the token** as its bearer credential (client env:
   `EMAILS_FLEET_TOKEN`); `emails auth whoami` shows the fleet principal, its
...
5. **Revocation** — two independent kill switches, both auditable:
   ...
   - Emails: `emails auth fleet revoke <sub>` sets `revoked_at` on the mapping
...
- Cost accepted: onboarding touches two CLIs. Mitigated by `emails auth fleet
  map` printing the exact `tenants` command when the referenced principal does
  not exist yet, and by a fleet skill composing both.

## Decision 2 — Scope model

Fleet tokens carry emails scopes in the shared `<app>:<action>` grammar
...
and no route grows a special fleet branch. Two constraints named for later,
not smuggled in now:
...
- **Send-as address restriction** (an agent may send only from
  ...
  token — a designed extension column on `fleet_principal_tenants`
  (`allowed_from_addresses`), enforced where send authority is already
  checked. Out of the first slices.

Role gates are unaffected: fleet principals are never `principalType:"user"`,
so human/session-only surfaces (member management, invitations, password
flows) remain unreachable with a fleet token regardless of scopes — same
containment the API-key class has today.

## Decision 3 — Audit events

Fleet auth reuses the structured, secret-free audit discipline of the API-key
...
| --- | --- | --- |
| `fleet.auth.allow` / `fleet.auth.deny` | every fleet-token authentication decision | outcome, sub, tid, jti, kid, reason, method, path, status, at |
| `fleet.map.create` | mapping row created | sub, emails tenant, granted by (user id / kid), note |
| `fleet.map.revoke` | mapping revoked | sub, emails tenant, revoked by, reason |
| `fleet.jwks.refresh` / `fleet.jwks.error` | JWKS cache refresh / fetch failure | url host, kid set, error class |
...
A new `@hasna/contracts/auth` fleet module, so tenants, emails, and every next
app share one definition instead of pinning copies:

- **Claims schema**: `FleetAccessTokenClaims` = `{ iss, aud, sub, tid, pt,
  scope, iat, exp, jti }` with `pt: "user" | "service"`.
- **Wire constants**: the fixed issuer string (`identities` today — renaming it
  is a coordinated fleet-wide change and stays out of scope), the algorithm
...
- **Structural detector** (is this bearer token a fleet JWS?) so credential
...
- **Documented mapping-table pattern**: each verifying app owns a
  `fleet_principal_tenants`-shaped table (`sub → app tenant`, `tid` pin,
