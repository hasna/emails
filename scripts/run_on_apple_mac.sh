...
# REMOTE_HOST. Also runs the internal legacy Swift smoke harness (MailerySmoke)
# on the Mac first so a broken core fails fast before assembling the bundle.
#
...
echo "==> smoke test on $REMOTE_HOST (internal legacy MailerySmoke target)"
