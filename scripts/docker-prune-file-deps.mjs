...
//
// The runtime image runs ONLY the HTTP service (`mailery-serve`) and the
// one-shot schema migration (`mailery db migrate`). Neither imports the
// workspace-local packages that are wired in via `file:` paths (e.g. the MCP
// harness, which is loaded lazily and only by the `mailery mcp` command). Those
