...
check(EmailsCLI.markReadArgs(id: "x", unread: false) == ["inbox", "mark-read", "x"], "mark-read argv")
check(EmailsCLI.markReadArgs(id: "x", unread: true) == ["inbox", "mark-read", "x", "--unread"], "mark-unread argv")
check(EmailsCLI.archiveArgs(id: "x", undo: false) == ["inbox", "archive", "x"], "archive argv")
check(EmailsCLI.archiveArgs(id: "x", undo: true) == ["inbox", "archive", "x", "--undo"], "unarchive argv")
check(EmailsCLI.starArgs(id: "x", undo: false) == ["inbox", "star", "x"], "star argv")
check(EmailsCLI.starArgs(id: "x", undo: true) == ["inbox", "star", "x", "--undo"], "unstar argv")
check(EmailsCLI.labelArgs(id: "x", label: "work", remove: false) == ["inbox", "label", "x", "work"], "label argv")
check(EmailsCLI.labelArgs(id: "x", label: "work", remove: true) == ["inbox", "label", "x", "work", "--remove"], "label remove argv")
check(EmailsCLI.replyArgs(id: "x", body: "hi there", html: false) == ["inbox", "reply", "x", "--body", "hi there"], "reply argv")
check(EmailsCLI.replyArgs(id: "x", body: "hi", html: true) == ["inbox", "reply", "x", "--body", "hi", "--html"], "reply html argv")
check(EmailsCLI.refreshArgs() == ["refresh"], "refresh argv")
check(
    EmailsCLI.sendArgs(to: ["a@b.com", "c@d.com"], subject: "Hi", body: "yo",
...
check(
    EmailsCLI.sendArgs(to: ["a@b.com"], subject: "S", body: "B", html: true)
