...
final class MailBridge {
    ...
    func argv(for payload: [String: Any]) -> [String]? {
        ...
        switch action {
        case "markRead": return MaileryCLI.markReadArgs(id: id, unread: bool(payload, "unread"))
        case "archive":  return MaileryCLI.archiveArgs(id: id, undo: bool(payload, "undo"))
        case "star":     return MaileryCLI.starArgs(id: id, undo: bool(payload, "undo"))
        case "label":    return MaileryCLI.labelArgs(id: id, label: str(payload, "label"), remove: bool(payload, "remove"))
        case "trash":    return MaileryCLI.labelArgs(id: id, label: "trash", remove: bool(payload, "undo"))
        case "spam":     return MaileryCLI.labelArgs(id: id, label: "spam", remove: bool(payload, "undo"))
        case "reply":    return MaileryCLI.replyArgs(id: id, body: str(payload, "body"), html: bool(payload, "html"))
        case "send":
            let from = str(payload, "from")
            return MaileryCLI.sendArgs(
        ...
        case "refresh":  return MaileryCLI.refreshArgs()
        default:         return nil
        }
    }
}
    ...
    func userContentController(_ ucc: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if action == "shareAttachment" {
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                let url = MaileryApp.firstURL(in: res.output)
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let result = MaileryCLI().run(args)
